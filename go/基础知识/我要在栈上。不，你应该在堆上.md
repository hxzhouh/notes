...
``` shell
PS C:\gowork\public\src\go-study\stack> go build -gcflags '-m -l' stack.go
# command-line-arguments
.\stack.go:11:54: &User literal escapes to heap
```

第二，通过反编译命令查看

```shell
go tool compile -S .\stack.go
...
这是因为 `GetUserInfo()` 返回的是`指针对象`，引用被返回到了方法之外了。因此编译器会把该对象分配到堆上，而不是栈上。否则方法结束之后，局部变量就被回收了，岂不是翻车。所以最终分配到堆上是理所当然的
